...
class BitBangConnection {
    ...
    startBench() {
        ...
        try {
            this._sendStreamSYN(streamId, FLAG_SYN, JSON.stringify({ bench: true }));
        } catch (e) {
            this.benchStreams.delete(streamId);
            this.benchVideo = null;
            console.error('[bench] could not start:', e.message || e);
            return;
        }
    ...
    handleBenchFrame(frame, st) {
        // An ordinary SWSP stream, so each frame holds receive credit until
        // it is counted -- which is right now.
        if (!(frame.flags & FLAG_SYN)) {
        ...
        if (frame.flags & FLAG_FIN) {
            ...
            this.flow.resetStream(frame.streamId, new Error('bench complete'));
            return;
        }
